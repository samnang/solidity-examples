...
describe("Solidity By Examples / Array", function () {
  ...
  beforeEach(async () => {
    const Contract = await ethers.getContractFactory("contracts/getting_started_with_solidity/13_array/MyContract.sol:MyContract");
  ...
  it("Should be able to verify all variables", async function () {
    ...
    expect(await contract.getLegth()).to.equal(4);
    ...
    expect(await contract.getLegth()).to.equal(3);
  });
});
