...
describe("Solidity By Examples / Multi Sig Wallet", function () {
  ...
  beforeEach(async () => {
    ...
    const Contract = await ethers.getContractFactory("contracts/solidity_by_examples/02_multi_sig_wallet/MultiSigWallet.sol:MultiSigWallet", owner1);
  ...
  it("Should allow only owners to be able to submit transactions", async function () {
    await expect(
      contract.connect(otherSigner).submitTransaction(
        otherSigner.address,
        100,
    ...
    await expect(
      contract.connect(owner1).submitTransaction(otherSigner.address, 100, ethers.constants.HashZero)
    ).to.emit(contract, "SubmitTransaction").withArgs(
      owner1.address,
      0,
      otherSigner.address,
      100,
  ...
  it("Should allow only owners to be able to confirm transactions", async function () {
    await contract.connect(owner1).submitTransaction(otherSigner.address, 100, ethers.constants.HashZero);

    await expect(
      contract.connect(otherSigner).confirmTransaction(0)
    ).to.be.revertedWith("not owner");

    await expect(
      contract.connect(owner1).confirmTransaction(1)
  ...
  it("Should be able to execute the transaction when it has enough confirmations", async function () {
    ...
    await expect(
      contract.connect(owner1).executeTransaction(0)
    ).to.be.revertedWith("cannot execute tx");

    await contract.connect(owner2).confirmTransaction(0);
    await expect(
      contract.connect(owner1).executeTransaction(0)
    ).to.emit(contract, "ExecuteTransaction").withArgs(owner1.address, 0);
  });
})
