...
describe("Sample Apps / Timelock", function () {
  ...
  beforeEach(async () => {
    ...
    const Contract = await ethers.getContractFactory("contracts/sample_apps/04_time_lock/Timelock.sol:Timelock", owner);
  ...
  it("Should be able to verify all variables", async function () {
    ...
    await expect(contract.connect(signer1).withdraw()).to.be.revertedWith("lock time has not expired");
