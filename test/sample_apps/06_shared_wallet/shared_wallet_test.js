...
describe("Sample Apps / Shared Wallet", function () {
  ...
  beforeEach(async () => {
    ...
    const Contract = await ethers.getContractFactory("contracts/sample_apps/06_shared_wallet/SharedWallet.sol:SharedWallet", owner);
  ...
  it("Should be able to withdraw fund", async function () {
    ...
    expect(await contract.provider.getBalance(contract.address)).to.equal(ethers.utils.parseEther("0.5"))

    await expect(contract.connect(signer2).withdraw(ethers.utils.parseEther("0.5"))).to.be.revertedWith("Not contract owner or owners of shared wallet");
  });
