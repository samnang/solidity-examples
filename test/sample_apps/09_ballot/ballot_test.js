...
describe("Sample Apps / Ballot", function () {
  ...
  beforeEach(async () => {
    ...
    const Contract = await ethers.getContractFactory("contracts/sample_apps/09_ballot/Ballot.sol:Ballot", chairperson);
  ...
  it("should only allow to vote by chairperson", async function () {
    ...
    await expect(
      contract.connect(allowedAccount).vote(proposals.indexOf("Apple"))
    ).to.be.not.reverted;
