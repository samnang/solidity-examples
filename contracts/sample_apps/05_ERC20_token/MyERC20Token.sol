...
interface ERC20 {
  ...
  function allowance(address _owner, address _sender) external view returns (uint remaining);
...
contract MyERC20Token is ERC20 {
  ...
  function balanceOf(address _address) public view override returns(uint) {
  ...
  function transferFrom(address _from, address _to, uint _value) public override returns(bool) {
    if (_value > 0 && __allowances[_from][msg.sender] > 0 && __allowances[_from][msg.sender] >= _value && !isContract(_to)) {
